...
class MyAdminSite(AdminSite):
    ...
    def get_urls(self):
        ...
        urls += [
            ...
            url(r'^rent_data/calculate_median_2bdr_rent/$', self.admin_view(self.calculate_median_2bdr_rent), name='calculate_median_2bdr_rent'),
    ...
    def calculate_median_2bdr_rent(self, request):

        rents = []

        for property in Property.objects.all():
            if property.current_2bdrm_rent is not "unknown":
                rents.append(property.current_2bdrm_rent)

        median_rent_stat = Stat.objects.get(name__exact = "median_2bdr_rent")
        median_rent_stat.value = statistics.median(rents)
        median_rent_stat.save()
